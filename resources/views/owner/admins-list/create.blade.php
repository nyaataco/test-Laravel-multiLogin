...
<h1 class="text-lg font-bold mb-10 lg:w-4/5 mx-auto">管理者 新規登録</h1>

<div class="mid:w-10/12 lg:w-4/5 mx-auto mb-10">

    <form method="post" action="{{ route('owner.admins-list.store') }}" class="space-y-10">
        @csrf

        <div>
            <label for="admin_id" class="block">管理者ID</label>
            <x-input-error :messages="$errors->get('admin_id')" class="mt-2" />
            <input type="number" name="admin_id" id="admin_id" value="{{ old('admin_id') }}" class="w-full lg:w-10/12">
        </div>
